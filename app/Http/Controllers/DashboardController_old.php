...
class DashboardController extends Controller
{
    ...
    public function generateContract($id)
    {
        ...
        $pdf = Pdf::loadView('Pages.Contract', compact('manlistEntry'))->setPaper('a4', 'portrait'); // Set A4 size
    ...
    public function generateForm($id)
    {
        ...
        // Load Blade view into PDF
        $pdf = Pdf::loadView('Pages.Form', compact('manlistEntry'))->setPaper('a4', 'portrait'); // Set A4 size
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('dashboard.index')->with('success', 'Employee successfully added!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('dashboard.edit', $manlist->id)->with('success', 'Employee information updated successfully!');
    }
        ...
        try {
            ...
            for ($i = 1; $i < count($rows); $i++) {
                ...
                $manlistData[] = array_map($upper, [
                    ...
                    'datehired' => $convertDate($row['DATEHIRED']),
                    'workbase' => $row['WORKBASE'],
                    'temporary_workbase' => $row['TEMPORARY_WORKBASE'],
                    'project_assigned' => $row['PROJECT_ASSIGNED'],
                    'project_hired' => $row['PROJECT_HIRED'],
                    'contract_expiration' => $convertDate($row['CONTRACT_EXPIRATION']),
                    'probitionary_date' => $convertDate($row['PROBITIONARY_DATE']),
                    'regularization_date' => $convertDate($row['REGULARIZATION_DATE']),
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
                ...
                $personalData[] = array_map($upper, [
                    'birthdate' => $convertDate($row['BIRTHDATE']),
                    'gender' => $row['GENDER'],
    ...
    public function destroy($id)
    {
        try {
            ...
            $entry->delete();
            return redirect()->route('dashboard.index')->with('success', 'Entry deleted successfully!');
        } catch (\Exception $e) {
